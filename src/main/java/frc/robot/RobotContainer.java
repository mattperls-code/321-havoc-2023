...
public class RobotContainer {
  ...
  public RobotContainer() {
    swerve.setDefaultCommand(
      swerve.drive(
        () -> kMaxSpeedMetersPerSecond * -MathUtil.applyDeadband(driver.getLeftY(), kJoystickDeadband),
        () -> kMaxSpeedMetersPerSecond * MathUtil.applyDeadband(driver.getLeftX(), kJoystickDeadband),
        () -> kMaxOmegaRadiansPerSecond * MathUtil.applyDeadband(driver.getRightX(), kJoystickDeadband),
        true, kGearRatio));
    ...
    //           var velo = driver.getLeftY() * kMaxSpeedMetersPerSecond;
    //           SmartDashboard.putNumber("controllerSetpointMetersPerSecond", velo);
    //           var state = new SwerveModuleState(velo, Rotation2d.fromRadians(0));
