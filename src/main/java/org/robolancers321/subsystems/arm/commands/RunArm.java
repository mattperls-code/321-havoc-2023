...
public class RunArm extends CommandBase {
  ...
  public void execute() {
    ...
    TrapezoidProfile.State anchorProfileState =
        arm.periodicIO.anchorProfile.calculate(
            Timer.getFPGATimestamp() - arm.periodicIO.anchorProfileStartTime);

    TrapezoidProfile.State floatingProfileState =
        arm.periodicIO.floatingProfile.calculate(
            Timer.getFPGATimestamp() - arm.periodicIO.floatingProfileStartTime);

    arm.periodicIO.anchorFF =
        Constants.Arm.Anchor.FF.ANCHOR_FEEDFORWARD.calculate(
            Math.toRadians(anchorProfileState.position), anchorProfileState.velocity);
    arm.periodicIO.floatingFF =
        Constants.Arm.Floating.FF.FLOATING_FEEDFORWARD.calculate(
            Math.toRadians(floatingProfileState.position), floatingProfileState.velocity);
